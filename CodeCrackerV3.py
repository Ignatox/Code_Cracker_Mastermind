...
#Variables globales para para persistencia
code_length = 4
...
def ia_dificil(intentos_previos, respuestas_previas, nro_intento, code_length=4):
    global posibles_por_posicion, fijas_confirmadas, descartados_globales, descartados_por_posicion
    
    print("IA: Dejame pensarlo...")
    
    #Procesar intentos y respuestas previas
...
#Confirmar números fijos                    
    for i in range(code_length):
        if len(posibles_por_posicion[i]) == 1:
            ...
            print(f"Numero fijo encontrado en posición {i}: {num_fijo}")
    ...
    #while True:
    if(nro_intento == 1):
        print(f"Intento nro: {nro_intento}")
